...
namespace LightPad.Frontend {
    ...
    public class AppItem : Gtk.EventBox {
        ...
        public AppItem (int size, double font_size, int box_width, int box_height) {
            ...
            //  this.enter_notify_event.connect ( () => { this.focus_in (); return true; } );
            //  this.leave_notify_event.connect ( () => { this.focus_out (); return true; } );
        }
        ...
        public new void focus_in () {
            message ("In focus item: %s", this.label);
        ...
        public new void focus_out () {
            ...
            //  message ("Out focus item: %s, has_focus: %s, current_frame: %d", this.label, this.has_focus.to_string (), this.current_frame);
            GLib.Timeout.add (((int)(1000/FPS)), () => {
                if (this.current_frame >= RUN_LENGTH || this.has_focus) {
                    current_frame = 1;
                    return false; // Stop animation
